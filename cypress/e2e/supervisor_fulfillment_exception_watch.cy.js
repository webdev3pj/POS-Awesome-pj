...
describe("Supervisor fulfillment exception workflow (watch mode)", () => {
  it("flags pick exception line-wise and performs supervisor override release", () => {
    ...
    startPosSessionForRole({
      roleStorageValue: "cline-Supervisor",
    ...
    cy.request(`${relayBase}/relay/pick-queue?pos_profile_id=${encodeURIComponent(profileName)}&limit=100`)
      ...
      .then(() => {
        cy.contains(".v-btn", "Flag Exception", { timeout: 30000 }).click({ force: true });
      })
      .then(() => cy.request(`${relayBase}/api/transactions/${encodeURIComponent(targetLocalSaleRef)}`))
      .then((exceptionResp) => {
        expect(exceptionResp.status).to.eq(200);
        expect(exceptionResp.body.ok).to.eq(true);
        const sale = exceptionResp.body && exceptionResp.body.sale ? exceptionResp.body.sale : {};
        expect(String(sale.pick_status || ""), "relay sale pick_status after exception").to.eq("PICK_EXCEPTION");
      ...
      .then((releasedResp) => {
        ...
        const sale = releasedResp.body && releasedResp.body.sale ? releasedResp.body.sale : {};
        expect(String(sale.pick_status || ""), "pick_status remains exception").to.eq("PICK_EXCEPTION");
        ...
        cy.writeFile("cypress/tmp/supervisor_exception_target.json", {
          ...
          cloud_invoice_name: sale.cloud_invoice_name || "",
          token_id: sale.token_id || "",
        });
        cy.log(`Supervisor exception override released: ${targetLocalSaleRef}`);
      });
  });
});
