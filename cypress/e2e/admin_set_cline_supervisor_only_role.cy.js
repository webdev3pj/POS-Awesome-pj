...
describe("Admin preflight: set cline to Supervisor-only operational role", () => {
  it("keeps non-cline roles but leaves only cline-Supervisor among cline-* roles", () => {
    let targetOperationalRole = "cline-Supervisor";
    ...
    frappeCall("frappe.client.get_list", {
      ...
      const exact = clineRoles.find((r) => r === "cline-Supervisor");
      const caseInsensitive = clineRoles.find(
        (r) => r.toLowerCase() === "cline-Supervisor".toLowerCase()
      );
      ...
      if (!resolved) {
        throw new Error(
            [
            "Precondition failed: No Supervisor cline role exists on this site.",
      ...
      cy.log(`Resolved Supervisor operational role: ${targetOperationalRole}`);
    });
