...
describe("Admin preflight: set cline to Picker-only operational role", () => {
  it("keeps non-cline roles but leaves only cline-Picker among cline-* roles", () => {
    let targetOperationalRole = "cline-Picker";
    ...
    frappeCall("frappe.client.get_list", {
      ...
      const exact = clineRoles.find((r) => r === "cline-Picker");
      const caseInsensitive = clineRoles.find(
        (r) => r.toLowerCase() === "cline-picker".toLowerCase()
      );
      const fuzzy = clineRoles.find((r) => {
        const t = r.toLowerCase();
        return t.includes("picker");
      });
      ...
      if (!resolved) {
        throw new Error(
            [
            "Precondition failed: No Picker cline role exists on this site.",
      ...
      cy.log(`Resolved Picker operational role: ${targetOperationalRole}`);
    });
