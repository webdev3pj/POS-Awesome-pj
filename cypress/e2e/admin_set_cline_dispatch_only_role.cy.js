...
describe("Admin preflight: set cline to Dispatch-only operational role", () => {
  it("keeps non-cline roles but leaves only cline-Dispatch among cline-* roles", () => {
    let targetOperationalRole = "cline-Dispatch";
    ...
    frappeCall("frappe.client.get_list", {
      ...
      const exact = clineRoles.find((r) => r === "cline-Dispatch");
      const caseInsensitive = clineRoles.find(
        (r) => r.toLowerCase() === "cline-dispatch".toLowerCase()
      );
      ...
      if (!resolved) {
        throw new Error(
            [
            "Precondition failed: No Dispatch cline role exists on this site.",
      ...
      cy.log(`Resolved Dispatch operational role: ${targetOperationalRole}`);
    });
