...
- `codes-4.3-dispatch` adds dispatch monitor UX polish (SLA emphasis, queue sorting, richer phase timing/detail timeline) and restores a repeatable local Docker staging validation loop (`pj.local:8080`) with separate local-staging Cypress specs.
...
- [ ] Dispatch monitoring-first UX polish (SLA thresholds/aging cues, timeline readability, sorting rationale labels) and analytics-oriented timing instrumentation. `Partial`: `codes-4.3-dispatch` now includes queue SLA chips, `Over SLA` summary, workflow-priority queue sorting, and richer phase timing/detail cards; threshold tuning and longer-run analytics persistence still pending.
...
- [x] Local-staging (`pj.local`) SA->Cashier->Picker->Dispatch + Supervisor wrapper suite exists under `cypress/e2e/local_staging/` and has been rerun successfully on `codes-4.3-dispatch` (2026-02-26), including dispatch monitor UI assertions and relay-backed fulfillment state updates.
